...
```{r load-data-locally, message=FALSE, warning=FALSE, eval=FALSE, include=FALSE}
# Load data for individual listings in paris
  # Set working directory to folder with csv files (ie, "data")
setwd("/Users/santiagogiordano/Documents/R_Stuff/GitHub_Repos/interactive-map-paris/data/")
  # Create list with csv files
paris_csvs <- list.files(path = "/Users/santiagogiordano/Documents/R_Stuff/GitHub_Repos/interactive-map-paris/data/", 
                         pattern = "*.csv")
  # Read files
paris_listings2 <- plyr::ldply(paris_csvs, read.csv) %>% 
    # Get rid of index column created by the loading function
    select(-X)
  # Reset working directory
setwd("/Users/santiagogiordano/Documents/R_Stuff/GitHub_Repos/interactive-map-paris/")
```
...
paris_nhoods_geodata <- st_read(here::here("data", 
                                           "paris_neighbourhoods.nosync.geojson")) %>% 
...
```{r}
# Aggregate number of listings and avergage prices by neighbourhood
paris_list_and_avgP_nhood <- paris_listings %>% 
                                ...
                                summarise(n_listings = n(), avg_price = mean(price)) %>% 
...
-   Webscrape data for population and area of each arrondissement (used to find listings by population and by area)

```{r webscrape-paris}
# Read html address with arrondissements data from wikipedia (in italian)
wiki_paris_it <- read_html("https://it.wikipedia.org/wiki/Arrondissement_municipali_di_Parigi")
...
paris_arronds_full <- merge(paris_list_and_avgP_nhood, paris_arronds_it, 
                            by.x = "neighbourhood_cleansed", by.y = "neighbourhood")
```
...
## Calculate listings per 1000 people
  # Get rid of empty space in population column and convert variable to numeric class
...
labels_n_listings <- sprintf(
  ...
  paris_merged_sf$neighbourhood_cleansed, paris_merged_sf$n_listings) %>% 
...
saveWidget(total_listings_map, file=paste0(getwd(), "/htmlwidgets/paris_tot_listings.html"))
...
labels_listings_pop <- sprintf(
  "<strong>%s</strong><br/> Listings per 1000 people: %g",
  paris_merged_sf$neighbourhood_cleansed, paris_merged_sf$listings_by_pop) %>% 
  lapply(htmltools::HTML)
...
# Save interactive map as widget
saveWidget(listings_by_pop_map, file=paste0(getwd(), "/htmlwidgets/paris_listings_by_pop.html"))
```
...
labels_listings_km2 <- sprintf(
  ...
  paris_merged_sf$neighbourhood_cleansed, paris_merged_sf$listings_km2) %>% 
  lapply(htmltools::HTML)
...
# Save interactive map as widget
saveWidget(listings_by_area_map, file=paste0(getwd(), "/htmlwidgets/paris_listings_by_area.html"))
...
labels_avg_price <- sprintf(
  ...
  paris_merged_sf$neighbourhood_cleansed, paris_merged_sf$avg_price) %>% 
  lapply(htmltools::HTML)
...
# Save interactive map as widget
saveWidget(listings_price_map, file=paste0(getwd(), "/htmlwidgets/paris_listings_price.html"))
```
